const airplaneServiceBase = {
  ...
  serviceAirplane: [
    { category: "Место прибытия", description: "Аэропорт Севастополя" },
    { category: "Дистанция", description: "1050 км" },
    { category: "Время в пути", description: "4 часа 50 минут" },
    { category: "Стоимость", description: "50.000 руб" },
  ],
};
export const serviceBannerFunc = () => {
  let indexSlide = 0;

  const containerSliderElem = document.getElementById(
    "container_slider_service_airplane"
  );
  const containerServiceElem = document.getElementById('container_service_plane_airplane')
  const slideImgElem = document.getElementById("img_slide_servise_airplane");
  const nameSlideElem = document.getElementById(
    "servise_airplane_name_airport"
  );
  ...
  function renderSlide() {
    ...
    function renderService() {
        airplaneServiceBase.serviceAirplane.forEach((elem)=>{
            containerServiceElem.insertAdjacentHTML('beforeend', `
            <div class="item_plane">
                <h4 class="upper_text prev_text">${elem.category}</h4>
                <h2 class="upper_text">${elem.description}</h2>
            </div>
          `)
        })
